...
const AuthScreen = ({ onViewChange, onLogin }: any) => {
    ...
    return (
        <div className="min-h-screen flex bg-surface-50 font-sans">
            {/* Left Side - Visual - REPLICATING IMAGE 02 DESIGN */}
            <div className="hidden lg:flex lg:w-1/2 bg-brand-900 relative overflow-hidden items-center justify-center p-12">
                ...
                <div className="relative z-10 flex flex-col items-center justify-center">
                    {/* Icon Container - Matching Image 02 rounded square style */}
                    ...
                    <p className="text-white/80 text-sm tracking-[0.25em] font-medium uppercase drop-shadow-md">
                        Gestão para Igrejas
                    </p>
                </div>
            </div>
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-8 lg:p-12 bg-white">
                <div className="w-full max-w-md space-y-8">
                    ...
                    <div className="space-y-4">
                        <Input placeholder="seu@email.com" icon={<Mail size={18} />} />
                        ...
                        <div className="flex justify-between items-center text-sm">
                          ...
                          <button onClick={() => onViewChange('forgot')} className="text-brand-600 font-medium hover:underline">Esqueceu a senha?</button>
                        </div>
    ...
    <div className="min-h-screen bg-surface-50 flex font-sans text-surface-900">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-surface-200 flex flex-col shadow-soft transition-transform duration-300 lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <nav className="flex-1 px-4 space-y-1 overflow-y-auto py-4 custom-scrollbar">
              ...
              ].map(item => <NavItem key={item.id} item={item} active={activeTab === item.id} onClick={() => { setActiveTab(item.id); setIsSidebarOpen(false); }} />)}
          ...
          <div className="p-4 border-t border-surface-100">
              ...
              <button onClick={() => setView('login')} className="w-full flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-rose-600 hover:bg-rose-50 rounded-xl transition-colors">
